...
Base.setMethod(function _createStructure(structure_type, type, class_name) {
	...
	if (type) {
		...
		if (type == 'string') {
			structure.super = this.runtime.global_scope.get('String');
		} else if (type == 'number') {
			structure.super = this.runtime.global_scope.get('Number');
		}
 ...
 * @version  0.1.0
 */
Base.setMethod(function createMap() {

	if (!this.runtime) {
		throw new Error('Unable to create map without a runtime');
	}

	let map = new Protodoc.HistoryMap();

	map.runtime = this.runtime;

	this.runtime.maps.push(map);
